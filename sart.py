...
class SART(object):
    def __init__(self):
        ...
        self.ITI = 0.9
        self.stimSizes_pt = [48, 72, 94, 100, 120] #in point as per original paper
        self.stimSizes_mm = [12, 18, 23, 24, 29] #in mm, maintaining original ratio
    ...
    def pressSpace(self, x, y):
    	self.space = self.instructionsFont.render("(Press spacebar when ready)", 1, (255,255,255))
    	self.screen.blit(self.space, (x, y))

    def displayTrial(self, i, data):
        #randomly choose font size
        self.sizeIndex = np.random.randint(0,5)
        self.stimulusFont = pygame.font.SysFont("arial", self.stimSizes_pt[self.sizeIndex])

        #set stimulus
        self.stimulusText = self.stimulusFont.render(str(data.at[i, "stimulus"]), 1, (255,255,255))
        ...
        while int(round(time.time() * 1000)) - self.baseTime <= 1150:
            for event in pygame.event.get():
                if event.type == KEYDOWN and event.key == K_SPACE:
                    self.keyPress = 1
                    data.set_value(i, 'RT', int(round(time.time() * 1000)) - self.baseTime)
            ...
            #display post stim mask for 900ms
                self.screen.blit(self.maskImage, ([self.screen_x/2 - self.maskX/2, self.screen_y/2 - self.maskY/2], [self.screen_x/2 + self.maskX/2, self.screen_y/2 + self.maskY/2]))
    ...
    def run(self):
        ...
        self.screen.blit(self.title, (self.screen_x/2 - self.titleW/2, self.screen_y/2 - 250))

        self.line1 = self.instructionsFont.render("Numbers will appear in the center of the screen.", 1, (255,255,255))
        self.screen.blit(self.line1, (100, self.screen_y/2 - 100))

        self.line2 = self.instructionsFont.render("Press the spacebar after you see a number.", 1, (255,255,255))
        self.screen.blit(self.line2, (100, self.screen_y/2))

        self.line3 = self.instructionsFont.render("However, if the number is a 3, do NOT press the spacebar.", 1, (255,255,255))
        ...
        while self.instructionsPractice:
        	...
        	self.screen.blit(self.background, (0,0))
        	self.practiceInstructions = self.instructionsFont.render("We will begin with a few practice trials...", 1, (255,255,255))
        ...
        #Practice trials
        self.practiceTrials = pd.DataFrame([5,7,7,3,9,2,1,3,8,6], columns=['stimulus'])
        ...
        while self.practiceEndScreen:
        	...
        	self.screen.blit(self.background, (0,0))
        	self.practiceEndLine = self.instructionsFont.render("We will now begin the main trials...", 1, (255,255,255))
        ...
        while self.endScreen:
        	...
        	self.screen.blit(self.background, (0,0))
        	self.endLine = self.instructionsFont.render("End of task.", 1, (255,255,255))
