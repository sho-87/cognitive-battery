...
class DigitspanBackwards(object):
    def __init__(self, blocks = 6):
        ...
        self.digitLengths = np.asarray([j for i in range(self.startLength, self.endLength+1) for j in [i, i]])
    ...
    def displayNumbers(self, i, data):
        ...
        for number in self.sequence:
            ...
            self.screen.blit(self.stimulusText, (self.screen_x/2 - self.stimulusW/2, self.screen_y/2 - self.stimulusH/2))
            pygame.display.flip()
    ...
    def numberEntry(self):
        ...
        while self.entry == True:
            ...
            self.entryInstructions = self.instructionsFont.render("Type the sequence in backwards order:", 1, (0,0,0))

            self.sequenceText = self.stimulusFont.render(''.join(self.userSequence), 1, (0,0,0))
            ...
            self.screen.blit(self.sequenceText, (self.screen_x/2 - self.sequenceTextW/2, self.screen_y/2 - self.sequenceTextH/2))
            pygame.display.flip()
    ...
    def run(self):
        ...
        self.title = self.instructionsFont.render("Backwards Digit Span", 1, (0,0,0))
        self.titleW = self.title.get_rect().width
        self.screen.blit(self.title, (self.screen_x/2 - self.titleW/2, self.screen_y/2 - 300))

        self.line1 = self.instructionsFont.render("You will be shown a number sequence, one number at a time.", 1, (0,0,0))
        self.screen.blit(self.line1, (100, self.screen_y/2 - 200))

        self.line2 = self.instructionsFont.render("Memorize the number sequence.", 1, (0,0,0))
        self.screen.blit(self.line2, (100, self.screen_y/2 - 100))

        self.line3 = self.instructionsFont.render("You will then be asked to type the sequence in reverse/backwards order. For example...", 1, (0,0,0))
        self.screen.blit(self.line3, (100, self.screen_y/2))

        self.line4 = self.instructionsFont.render("Sequence: 1 2 3 4 5", 1, (0,0,0))
        self.line4W = self.line4.get_rect().width
        self.screen.blit(self.line4, (self.screen_x/2 - self.line4W/2, self.screen_y/2+100))

        self.line5 = self.instructionsFont.render("Correct: 5 4 3 2 1", 1, (0,0,0))
        self.line5W = self.line5.get_rect().width
        self.screen.blit(self.line5, (self.screen_x/2 - self.line5W/2, self.screen_y/2+150))

        self.line6 = self.instructionsFont.render("The sequences will get longer throughout the experiment.", 1, (0,0,0))
