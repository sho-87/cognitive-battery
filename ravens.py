...
class Ravens(object):
    def __init__(self, start = 1, numTrials = 12):
        ...
        self.dirImages = [ f for f in listdir(self.imagePath) if isfile(join(self.imagePath,f)) ]
        ...
        #load practice image
        self.practiceImage = pygame.image.load(self.imagePath + "practice\\practice.png")
        ...
        self.img_example_answers = pygame.image.load(self.imagePath + "practice\\example_answers.png")
    ...
    def displayTrial(self, i, data, type):
        ...
        while int(round(time.time() * 1000)) - self.baseTime < self.stimDuration:
            ...
            for event in pygame.event.get():
                if event.type == KEYDOWN and event.key == K_F12:
                    ...
                    if event.key == K_1:
                        ...
                        data.set_value(i, 'RT', str((float(self.endTime) - float(self.baseTime)) / 1000))
                        return 0
                    elif event.key == K_2:
            ...
            self.screen.blit(self.curImage, (self.screen_x/2 - self.stimW/2, self.screen_y/2 - self.stimH/2))
            ...
            self.timerText = self.instructionsFont.render("Time left: " + str(self.timer), 1, (0,0,0))
            self.timerW = self.timerText.get_rect().width
            self.screen.blit(self.timerText, (self.screen_x/2 - self.timerW/2, self.screen_y/2 + 400))
    ...
    def run(self):
        ...
        self.title = self.instructionsFont.render("Raven's Progressive Matrices", 1, (0,0,0))
        self.titleW = self.title.get_rect().width
        self.screen.blit(self.title, (self.screen_x/2 - self.titleW/2, self.screen_y/2 - 400))

        self.line1 = self.instructionsFont.render("You will see a grid of items with one item missing:", 1, (0,0,0))
        self.screen.blit(self.line1, (100, self.screen_y/2 - 350))

        self.screen.blit(self.img_example, (self.screen_x/2 - self.exampleW/2, self.screen_y/2 - 300))

        self.line2 = self.instructionsFont.render("There will be a set of 8 possible answer options:", 1, (0,0,0))
        self.screen.blit(self.line2, (100, self.screen_y/2 - 100))

        self.screen.blit(self.img_example_answers, (self.screen_x/2 - self.exampleAnswersW/2, self.screen_y/2 - 50))

        self.line3 = self.instructionsFont.render("Determine which option is the missing item.", 1, (0,0,0))
        self.screen.blit(self.line3, (100, self.screen_y/2 + 150))

        self.line4 = self.instructionsFont.render("In example above, the correct answer is 4.", 1, (0,0,0))
        self.screen.blit(self.line4, (100, self.screen_y/2 + 180))

        self.line5 = self.instructionsFont.render("Select your answer by pressing the corresponding number on the keyboard.", 1, (0,0,0))
        self.screen.blit(self.line5, (100, self.screen_y/2 + 250))

        self.line6 = self.instructionsFont.render("You will have 1 minute to complete each question.", 1, (0,0,0))
        ...
        while self.instructionsPractice:
        	...
        	self.screen.blit(self.background, (0,0))
        	self.practiceInstructions = self.instructionsFont.render("We will begin with a practice trial...", 1, (0,0,0))
        ...
        if self.practiceData.at[0, 'userAnswer'] == '2':
            ...
            self.feedbackLine = self.instructionsFont.render("Incorrect", 1, (255,0,0))

        self.feedbackLineH = self.feedbackLine.get_rect().height
        self.feedbackLineW = self.feedbackLine.get_rect().width

        self.screen.blit(self.feedbackLine, (self.screen_x/2 - self.feedbackLineW/2, self.screen_y/2 - self.feedbackLineH/2))
        ...
        for i in range(self.numTrials):
            ...
            if self.allData.at[i, 'userAnswer'] == str(self.allData.at[i, 'correctAnswer']):
        ...
        #rearrange dataframe
        self.columns = ['trial', 'image', 'correctAnswer', 'userAnswer', 'correct', 'RT']
