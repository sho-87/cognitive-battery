...
class ANT(object):
    def __init__(self, blocks = 6):
        ...
        self.preStimInterval = np.array([300, 300, 300, 550, 800, 1050, 1550, 2300, 3300, 4800, 6550, 11800])

        self.ITI = np.array([3000, 3250, 3500, 3750, 4000, 4500, 5000, 5500, 6500, 8000, 10000, 15000])

        #Create level combinations
        self.combinations = list(product(self.congruencyLevels, self.cueLevels, self.locationLevels))

        #level combinations give us 12 trials. Multiply by 3 to get the desired 36 trials per block
    ...
    def createBlock(self, blockNum, combinations, preStimInterval, ITI):
        ...
        self.curBlock = pd.DataFrame(data = combinations, columns=('congruency', 'cue', 'location'))
    ...
    def displayTrial(self, trialNum, data):
        ...
        self.baseTime = int(round(time.time() * 1000))
        while int(round(time.time() * 1000)) - self.baseTime < data.at[trialNum, "preStimInterval"]:
            pass
        ...
        self.displayFlanker(trialNum, data, data.at[trialNum, "congruency"], data.at[trialNum, "location"])
        ...
        self.baseTime = int(round(time.time() * 1000))
        while int(round(time.time() * 1000)) - self.baseTime < (data.at[trialNum, "ITI"] - self.rt):
            pass

    def runBlock(self, blockNum, totalBlocks, type):
        #if main trials, generate full blocks based on actual trial combinations
        if type == "main":
            self.curBlock = self.createBlock(blockNum, self.combinations, self.preStimInterval, self.ITI)
        #if practice trials, create 2 predetermined trials
        elif type == "practice":
            self.curBlock = pd.DataFrame(
            [["congruent", "spatial", "top", 3000, 6000],["incongruent", "center", "top", 2000, 3000],["incongruent", "spatial", "bottom", 3000, 4000],["congruent", "nocue", "top", 2000, 3000]],
            columns=('congruency', 'cue', 'location', 'preStimInterval', 'ITI')
            )
    ...
    def run(self):
        ...
        self.line5 = self.instructionsFont.render("Sometimes, a star will appear before the arrows:", 1, (0,0,0))
        self.screen.blit(self.line5, (100, self.screen_y/2 + 150))

        self.screen.blit(self.img_cue, (self.screen_x/2 - self.fixationW/2, self.screen_y/2 + 200))

        self.line6 = self.instructionsFont.render("The star will sometimes indicate where the arrows will appear.", 1, (0,0,0))
        ...
        self.columns = ['trial', 'block', 'congruency', 'cue', 'location', 'preStimInterval', 'ITI', 'direction', 'response', 'correct', 'RT']
